...
def main():
    ...
    if len(sys.argv) != 5:
        print("Usage: python script.py <csv_file_path> <output_dir> <model_class> <model_type>")
        sys.exit(1)
    ...
    # Define model paths and prefixes based on the user input
    model_prefix = ""
    if model_class == "bert":
        if model_type == "bert-large-japanese" or model_type == "bert-base-japanese":
            model_prefix = "cl-tohoku/"
        else:
            print("Invalid model_type for BERT. Please choose 'bert-large-japanese' or 'bert-base-japanese'.")
            sys.exit(1)
    elif model_class == "roberta":
        if model_type == "roberta-large-japanese" or model_type == "roberta-base-japanese":
            model_prefix = "nlp-waseda/"
        else:
            print("Invalid model_type for RoBERTa. Please choose 'roberta-large-japanese' or 'roberta-base-japanese'.")
            sys.exit(1)
    else:
        print("Invalid model_class. Please choose 'bert' or 'roberta'.")
        sys.exit(1)

    # Load tokenizer and model based on the specified model type
    ...
    # Silhouette analysis
    sil_scores = []
    for n_clusters in range(2, 11):  # Update the loop range to consider k from 2 to 10
    ...
    plt.figure()
    plt.plot(range(2, 11), sil_scores)  # Update x-axis range to match the loop range
    plt.title('Silhouette Score')
    plt.xlabel('Number of clusters (k)')
    plt.ylabel('Silhouette Score')
    plt.xticks(range(2, 11))  # Ensure ticks for each value of k
    plt.grid(True)
